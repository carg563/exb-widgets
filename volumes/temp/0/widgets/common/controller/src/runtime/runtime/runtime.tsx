...
const useWhetherWidgetVisible = (id: string): boolean => {
  return useSelector((state: IMState) => {
    const appConfig = state.appConfig;
    const browserSizeMode = state.browserSizeMode;
    const sectionNavInfos = state.appRuntimeInfo.sectionNavInfos;
    const info = searchUtils.getContentContainerInfo(appConfig, id, LayoutItemType.Widget, browserSizeMode);
    const activeViews = Object.keys(sectionNavInfos || {}).map(sId => sectionNavInfos[sId].currentViewId);
    let visible = true;

    if (info && info.type === ContainerType.View) {
      visible = activeViews.includes(info.id);
    }
    return visible;
  });
}
